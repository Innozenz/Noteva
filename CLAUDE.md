...
**The palette is "ink on paper" ("Atelier"): warm paper neutrals, a deep pine-petrol primary, a terracotta accent.** Music is black ink on cream, and the engraved score on the landing page already commits to that world — so the neutrals are a warm ivory (`--background: #faf8f4`, ink `--foreground: #1f1b16`), not the cool grey of a generic SaaS. **The primary (`#0b5a5f`) is deliberately dark and slightly blue-leaning, and that is the whole point of its value:** the green–teal arc of the wheel is already crowded — the *winds* family is `#0f766e` (an earlier primary candidate sat exactly there), and `success`/`percussion` own the greens — so a franker spruce would collide with three things at once. Darkening and pushing it toward petrol is what dodges all of them while still reading as evergreen. The accent is terracotta (`#cf4b2c`), complementary to the pine — the atelier and the clay — and stays rare on the same reasoning as before. **The eight instrument-family tokens were left untouched by the refonte**: they are the most finished part of the palette, and a primary that no longer sits *on* a family (the old indigo primary was exactly the keyboard family) is a small improvement, not a loss.

Because a token can only reach code that reads CSS variables, the primary and accent are also **hardcoded in four places that cannot read tokens, and every palette change must track them**: the favicon (`app/icon.svg`, plus the generated `app/favicon.ico` and `app/apple-icon.png` — a static SVG/ICO has no access to `var()`), the landing-page aurora and spotlight gradients (inline `rgb(...)` in `app/page.tsx`), and `app/global-error.tsx`, whose styles are inline **because the stylesheet may not have loaded** when it renders. Grep for the old hex before assuming a colour change is complete.
...
The terracotta `--accent` is deliberately **rare** — currently the hero underline alone, even now that the landing page is in colour. It is the one hue that names no category, so it only means something while it stays scarce.
