...
**The palette is "ink on paper" ("ReChord"): warm ivory neutrals, a spruce primary, a bronze accent.** It came from a tweakcn theme (a shadcn registry item, `theme/link.txt`), but was **transposed by hand, never applied with `shadcn add`** — that command rewrites `globals.css` with shadcn's own vocabulary (`--card`, `--secondary`, `--muted`-as-background, `--accent`-as-hover, `--chart-*`, `--sidebar-*`) and adds a `.dark` block, which would silently drop our `--surface*`/`--family-*`/status tokens and break light-only. The reference export sits in `theme/` for provenance and is imported by nothing. **The neutrals carry no brand hue** — warm ivory (`--background: #faf8f4`, ink `--foreground: #1f1b16`, borders and muted greys all warm), not the cool grey of a generic SaaS. The theme's own neutrals were green-tinted, and it was a mistake worth recording: once the editorial refonte put hairlines and muted labels on every page, that green tint washed the whole app green — the exact failure of *"a hue names something"*, the neutral drowning the brand. Green now lives only where it means something (the primary, the families, `success`); the paper stays neutral. **The primary is spruce `#2d765b`, and it deliberately sits *in* the green–teal arc that the families also occupy** (`winds #0f766e`, `percussion #15803d`, `success #059669`). The earlier "Atelier" palette went dark-petrol precisely to dodge that arc; here the collision is *accepted by decision*, on the reasoning that the primary dresses buttons and links while the families dress chips and noteheads — different contexts, so they cross without being read as the same code. The accent is bronze `#a66f3f` (the theme's `secondary`): the tweakcn theme has no editorial-emphasis hue of its own — its `accent` is a pale-mint hover background — so the bronze is repurposed as the one warm counterpoint, and stays rare. **The eight instrument-family tokens are left untouched**, as they were through every refonte: the most finished part of the palette.

Because a token can only reach code that reads CSS variables, the primary and accent are also **hardcoded in four places that cannot read tokens, and every palette change must track them**: the favicon (`app/icon.svg` fill `#2d765b`, plus the generated `app/favicon.ico` and `app/apple-icon.png` regenerated from it — a static SVG/ICO has no access to `var()`), the landing-page aurora and spotlight gradients (inline `rgb(...)` in `app/page.tsx` — primary `rgb(45 118 91)`, accent `rgb(166 111 63)`), and `app/global-error.tsx`, whose styles are inline **because the stylesheet may not have loaded** when it renders. Grep for the old hex before assuming a colour change is complete.
...
Typography: **Inter for body, Fraunces (a display serif) for `h1`–`h3` only**, wired through `--font-sans-custom` / `--font-display` — both from the tweakcn theme. The heading `letter-spacing` is kept light (`-0.015em`, not the grotesque's `-0.025em`): a serif tightened as hard as a grotesque runs its serifs into each other. Note the boilerplate had a bug worth not reintroducing — `globals.css` hardcoded `font-family: Arial` on `body`, silently overriding the font `next/font` had loaded. The theme also names JetBrains Mono, but nothing in the app uses `font-mono`, so it is not loaded.

**Instrument families own the colour** (`--family-*`, eight pairs of tokens; `lib/instruments/family.ts` maps them to labels and class strings). This generalises the rule the agenda arrived at the hard way — *neutrals belong to the grid, hues belong to the content* — into the one rule the whole palette follows: **a hue names something.** Family tokens name a family, status tokens (`success`/`warning`/`danger`) name a state, greys are layout, and bronze `--accent` is editorial emphasis. Nothing may take a hue merely because it looks better in colour; that is what makes the colours readable as information rather than decoration.
...
The landing page had a voice — oversized display type, small tracked eyebrows, hairline-separated rows instead of boxed cards, asymmetry — but it **stopped at the landing page**: every inner route fell back to default shadcn card grids, which is what made the app read "standard". `components/editorial.tsx` extracts that voice into shared, server-safe primitives so every page speaks it: `Eyebrow`, `PageTitle` (Fraunces, `size: "display"` for public/discovery pages, `"page"` for in-app screens under a nav bar), `PageHeader` (eyebrow + title + asymmetric right-aligned meta, closed by a hairline), `SectionTitle` (notehead dot + tracked-caps label + rule to the edge, with an optional `trailing` slot for counts), and `RowList`/`Row` (the landing "répertoire" pattern — `divide-y divide-border border-y`, hover washes the background instead of drawing a box).
...
The bronze `--accent` is deliberately **scarce and load-bearing** — it names the *editorial label*: the hero underline, every page/section `Eyebrow`, and the notehead dot on every `SectionTitle` (`components/editorial.tsx`). That is its whole job — the one warm counterpoint to a green palette, and the recurring warm note that stops the neutrals-plus-primary from reading all-green. It names no product category, so it must stay on labels and emphasis; spend it on anything else and it stops meaning "look here".
...
- Reviews are moderated **a posteriori** at `/admin/avis`, and **nobody is an admin yet**: promotion is a manual `UPDATE "user" SET role = 'ADMIN' WHERE email = '…'`. Until you run it, the screen exists but nobody can open it, and a teacher's report is filed with nobody notified. See the Reviews section.
