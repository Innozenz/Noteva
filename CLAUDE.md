...
**The palette is "ink on paper" ("ReChord"): green-tinted paper neutrals, a spruce primary, a bronze accent.** It came from a tweakcn theme (a shadcn registry item, `theme/link.txt`), but was **transposed by hand, never applied with `shadcn add`** — that command rewrites `globals.css` with shadcn's own vocabulary (`--card`, `--secondary`, `--muted`-as-background, `--accent`-as-hover, `--chart-*`, `--sidebar-*`) and adds a `.dark` block, which would silently drop our `--surface*`/`--family-*`/status tokens and break light-only. The reference export sits in `theme/` for provenance and is imported by nothing. Music is ink on paper, so the neutrals are a faintly green-washed ivory (`--background: #fbfbf9`, ink `--foreground: #0f2419` — a very dark green, not warm brown), not the cool grey of a generic SaaS. **The primary is spruce `#2d765b`, and it deliberately sits *in* the green–teal arc that the families also occupy** (`winds #0f766e`, `percussion #15803d`, `success #059669`). The earlier "Atelier" palette went dark-petrol precisely to dodge that arc; here the collision is *accepted by decision*, on the reasoning that the primary dresses buttons and links while the families dress chips and noteheads — different contexts, so they cross without being read as the same code. The accent is bronze `#a66f3f` (the theme's `secondary`): the tweakcn theme has no editorial-emphasis hue of its own — its `accent` is a pale-mint hover background — so the bronze is repurposed as the one warm counterpoint, and stays rare. **The eight instrument-family tokens are left untouched**, as they were through every refonte: the most finished part of the palette.
...
The bronze `--accent` is deliberately **rare** — currently the hero underline alone, even now that the landing page is in colour. It is the one warm hue in a green palette and names no category, so it only means something while it stays scarce.
